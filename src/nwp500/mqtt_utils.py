...
def redact_topic(topic: str) -> str:
    ...
    """
    # Extra safety: catch any remaining hexadecimal sequences of typical
    # MAC/device length
    # (Handles without delimiters, with colons, with hyphens,
    # uppercase/lowercase, etc.)
    for pattern in _MAC_PATTERNS:
        topic = pattern.sub("REDACTED", topic)
    # Defensive: Generic cleanup for sequences of 12+ hex digits that look like
    # MACs or IDs
    topic = re.sub(
