    ...
    async def set(self, device_mac: str, features: "DeviceFeature") -> None:
        ...
        async with self._lock:
            ...
            field_names = (
                ...
                if hasattr(features, "model_fields")
                else "N/A"
            )
            _logger.debug(f"Cached device info for {device_mac}: {field_names}")
