...
class MqttSubscriptionManager:
    ...
    def __init__(
        ...
        self._message_handlers: dict[str, list[Callable[[str, dict[str, Any]], None]]] = {}
    ...
    def _topic_matches_pattern(self, topic: str, pattern: str) -> bool:
        ...
        Examples:
            >>> _topic_matches_pattern("cmd/52/device1/status", "cmd/52/+/status")
            True
            >>> _topic_matches_pattern("cmd/52/device1/status/extra", "cmd/52/device1/#")
            True
        """
        # Handle exact match
        if topic == pattern:
    ...
    async def subscribe_device_status(
        ...
        def status_message_handler(topic: str, message: dict[str, Any]) -> None:
            ...
            try:
                ...
                self._schedule_coroutine(self._event_emitter.emit("status_received", device_status))
    ...
    async def _detect_state_changes(self, status: DeviceStatus) -> None:
        ...
        try:
            ...
            if status.errorCode and not prev.errorCode:
                await self._event_emitter.emit("error_detected", status.errorCode, status)
    ...
    async def subscribe_device_feature(
        ...
        def feature_message_handler(topic: str, message: dict[str, Any]) -> None:
    ...
    async def subscribe_energy_usage(
        ...
        response_topic = f"cmd/{device_type}/{self._client_id}/res/energy-usage-daily-query/rd"
